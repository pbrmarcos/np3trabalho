...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const hasPreferencesConsent = React.useCallback(() => {
  ...
  const setOpen = React.useCallback(
    ...
    [setOpenProp, open, hasPreferencesConsent],
  );
  ...
  return (
    ...
    >
      ...
      >
        <div
          data-sidebar="sidebar"
          className="flex h-full w-full flex-col bg-sidebar transition-all duration-300 ease-in-out group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
        >
          {children}
        </div>
      </div>
  ...
  ({ className, onClick, ...props }, ref) => {
    ...
    return (
      <Button
        ...
        variant="ghost"
        size="icon"
        className={cn("h-7 w-7 transition-transform duration-200 hover:scale-105 active:scale-95", className)}
        onClick={(event) => {
          onClick?.(event);
          toggleSidebar();
        }}
